"""Persona data loading from frontend."""
...
# Persona data mirrored from frontend
PERSONAS_DATA: Dict[str, List[Dict]] = {
    'New York': [
        {
            'id': 'ny-1',
            'name': 'Sarah Mitchell',
...
def get_all_personas() -> List[PersonaResponse]:
    """Get all personas as PersonaResponse objects."""
    personas = []
    for location, location_personas in PERSONAS_DATA.items():
        for persona_dict in location_personas:
            persona = PersonaResponse(
                id=persona_dict['id'],
                name=persona_dict['name'],
                title=persona_dict['title'],
                location=persona_dict['location'],
                industry=persona_dict['industry'],
                expertise=persona_dict['expertise'],
                experience=persona_dict['experience'],
                bio=persona_dict['bio'],
                insights=persona_dict.get('insights', []),
...
def get_persona_by_id(persona_id: str) -> Optional[PersonaResponse]:
...
def get_personas_by_location(location: str) -> List[PersonaResponse]:
    """Get personas for a specific location."""
    if location not in PERSONAS_DATA:
        return []
    
    personas = []
    for persona_dict in PERSONAS_DATA[location]:
        persona = PersonaResponse(
            id=persona_dict['id'],
            name=persona_dict['name'],
            title=persona_dict['title'],
